...
CUDA_COMPILE_FLAGS = -Wall -Wextra -O3 -I $(CUDA_DIR)/samples/common/inc -std=c++11 # --gpu-architecture=compute_60 --gpu-code=sm_60,compute_60
CUDA_LINK_FLAGS = -lcuda -lnvrtc -lcudart -L $(CUDA_DIR)/lib64


.PHONY: all $(SUBDIRS) coverage clean coveralls test
# $(SUBDIRS):
# 	$(MAKE) -C $@
...
all: dub.json libcxxdriver.so
	dub --build=release

test: libcxxdriver.so
